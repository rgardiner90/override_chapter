...
```{r}
overrides %>%
  group_by(state_abb, court_lower) %>%
  count() %>%
  ggplot() +
  geom_point(aes(x = court_lower, y = n)) +
...
overrides %>%
  group_by(state_abb, court_upper) %>%
  count() %>%
  ggplot() +
  geom_point(aes(x = court_upper, y = n)) +
...
```{r}
ggplot(data = overrides, aes(x = court_upper, y = time_to_override)) +
  geom_point() +
  geom_smooth() +
  xlab("Distance between Court and Upper Chamber") +
...
ggplot(data = overrides, aes(x = court_lower, y = time_to_override)) +
  geom_smooth() +
  geom_point() +
  xlab("Distance between Court and Lower Chamber") +
  ylab("Time between Decision and Override")
...
1. What to do about missind data for ideology?
